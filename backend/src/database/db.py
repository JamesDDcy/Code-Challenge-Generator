# Our helper functions to interact with the db
from sqlalchemy.orm import Session
from datetime import datetime, timedelta
from . import models

def get_challenge_quota(db: Session, user_id: str):
    # We query on the ChallengeQuota model
    # We filter it using the user_id
    # Then we get the first entry that matches the id
...
def create_challenge_quota(db: Session, user_id: str):
    ...
    db.add(db_quota)
    db.commit()
    # You need db.refresh to update an object with the latest data from the database after you make changes - for datetime
    db.refresh(db_quota)
    return db_quota
...
def get_user_challenge(db: Session, user_id: str):
    # We return all the challenges of the user
